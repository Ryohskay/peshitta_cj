...
def find_mislabels(
    ...
    if test_x is not None and isinstance(test_x, np.ndarray):
        # give the contents of mislabelled verses
        mislabel_verses = test_x[idcs]
    elif test_x is not None:
        # give the contents of mislabelled verses
        # in case test_x is not np array
        verses = []
        for idx in idcs:
            verses.append(test_x[idx])
...
def predict(
    classifier: type, test_X: np.ndarray | list, test_labels: np.ndarray
...
def predict_proba(
    classifier: type, test_X: np.ndarray | list, test_labels: np.array
...
def get_top_n_grams(
    ...
    top_k: int = 150,
    save_file: None | str = None,
) -> (np.array | None, np.array, np.array):
...
def split_list(lis: list, parts: int = 5) -> list[list]:
    ...
    if len(lis) % parts != 0:
        print(
            f"NOTE: the number of training samples ({len(lis)}) is not divisible by {parts}."
        )
