...
class TestDataSplit:
    ...
    def test_get_samples(
        self, etcbc_chr_verses: list[Verse], cal_romans_verse: Verse
    ) -> None:
    ...
    def test_generate_syriac_hf_raises(
        self,
        cal_ds: DataSplit,
    ) -> None:
        ...
        with pytest.raises(
            ...
            match=(
                ...
                + re.escape(" is empty!")
                + re.escape(
                    " Use generate_translit_hf() instead to get"
    ...
    def test_generate_syriac_hf(
        ...
        assert hf_lines_0[0] == {
            "label": 0,
            "text": " ".join(etcbc_chr_verses[0].get_syriac_words()),
        }
        assert hf_lines_0[1] == {
            "label": 0,
            "text": " ".join(etcbc_chr_verses[1].get_syriac_words()),
        }
        assert hf_lines_0[2] == {
            "label": 0,
            "text": " ".join(etcbc_chr_verses[2].get_syriac_words()),
        }
        # for NT
        hf_lines_1 = list(etcbc_ds.generate_syriac_hf(1))
        assert hf_lines_1[0] == {
            "label": 1,
            "text": " ".join(etcbc_acts_verse.get_syriac_words()),
        }
        assert hf_lines_1[1] == {
            "label": 1,
            "text": " ".join(etcbc_cor1_verse.get_syriac_words()),
        }
    ...
    def test_generate_translit_hf(
        ...
        assert hf_lines_1[0] == {
            "label": 0,
            "text": " ".join(cal_verse.get_translit_words()),
        }
        assert hf_lines_1[1] == {
            "label": 0,
            "text": " ".join(cal_one_word_verse.get_translit_words()),
        }
        # for NT
        hf_lines = list(cal_ds.generate_translit_hf(1))
        assert hf_lines[0] == {
            "label": 1,
            "text": " ".join(cal_romans_verse.get_translit_words()),
        }
        # Check it works with no params for both labels
        assert translits[0] == {
            "label": 0,
            "text": ("br$yt br) )lh) yt $my)" + " w_ yt )r()"),
        }
...
class TestLoadedDataset:
    ...
    def save_json(self, loaded_etcbc: LoadedDataset) -> None:
